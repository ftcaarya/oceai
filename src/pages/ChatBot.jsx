...
const MODES = [
  { id: 'policy', label: 'Policy', prompt: 'You are an expert ocean policy advisor. Help users understand ocean policies, regulations, and marine law. Be specific and cite relevant frameworks.' },
  { id: 'research', label: 'Research', prompt: 'You are an expert ocean researcher. Provide detailed information about marine biology, oceanography, climate impact, and conservation. Be scientific and accurate.' },
]
...
function MessageBubble({ msg, isUser }) {
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fade-up`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-accent/20 to-teal/20 border border-accent/10 flex items-center justify-center mr-3 mt-1 flex-shrink-0">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="2" strokeLinecap="round">
...
function TypingIndicator() {
  return (
    <div className="flex justify-start animate-fade-in">
      <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-accent/20 to-teal/20 border border-accent/10 flex items-center justify-center mr-3 flex-shrink-0">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="2" strokeLinecap="round">
          <path d="M2 12C2 12 5 4 12 4C19 4 22 12 22 12" />
          <path d="M2 12C2 12 5 20 12 20C19 20 22 12 22 12" />
        </svg>
...
export default function ChatBot() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: 'Connection failed. Make sure the backend is running.' }])
    } finally {
      setLoading(false)
    }
  }
            ...
            {messages.length === 0 && (
              <div className="h-full flex items-center justify-center">
                <div className="text-center max-w-sm">
                  <div className="w-12 h-12 rounded-2xl bg-accent/10 border border-accent/10 flex items-center justify-center mx-auto mb-4">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="1.5" strokeLinecap="round">
